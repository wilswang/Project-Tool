...
- [動態字段使用指南](dynamic-fields-guide.md) - 如何使用自定義字段功能（v1.1.0+）
- [Token 優化指南](token-optimization-guide.md) - 性能優化詳情
- [返回項目概覽](../README.md) - ProjectTool 工具集總覽
...
> ⚠️ **本文件已過時**：只涵蓋工具 A / B，缺少 **工具 C (Jira Tool)** 與 **工具 D (Sheet Tool)** 的
> 完整說明，版本歷史也停在 v1.2.4（目前為 v1.5.0）。
> **請以 [../README.md](../README.md) 為準**，那裡有四個工具的完整文件。
...
詳細使用說明請參考：[動態字段使用指南](dynamic-fields-guide.md)
...
> **v1.4.0+：上表的值由 `ProjectTool/config/env-values.json` 提供，不再寫死在 JAR 裡。**
> 改值、或新增第 4 個環境（如 `PROD`），只要改該檔 + 在 `files[].environments` 加上名稱，
> 不需要重新打包 JAR。完整說明（三層優先序、保留字、缺檔行為）見 `ProjectTool/README.md`
> 的「環境值外部化」章節。
...
v1.4.0 起，填值後若仍有殘留的 `{$...}` placeholder，工具會**在寫檔之前**中止該檔並讓整批 exit 1。
舊版會把字面 `{$typo}` 寫進 `.sql` 且照樣印 ✅、exit 0，導致 `SQL-processing.sh` 拿到壞檔。
...
| `envValues`                  | object | 否 | 逐單的環境值覆寫（v1.4.0+），形狀 `{"<ENV>": {"<key>": "<value>"}}`，優先序高於 `config/env-values.json` |
...
- **詳細指南**：參考[動態字段使用指南](dynamic-fields-guide.md)
...
### v1.2.4 (2026-02-11)
- 📦 **JAR 重命名**: `Project-Tool-1.2.4-jar-with-dependencies.jar` → `Project-Tool.jar`，移除版本號耦合
- 🔧 同步更新所有腳本與文檔中的 JAR 引用
